...
export default function Dashboard() {
  ...
  const [activityFeed, setActivityFeed] = useState([
    {
      time: "14:23:01",
      msg: "Brute force attempt blocked from 185.220.101.1",
      severity: "high" as const,
    },
    {
      time: "14:21:45",
      msg: "Port scan detected on ports 22-1024",
      severity: "medium" as const,
    },
    {
      time: "14:20:12",
      msg: "SSL certificate renewed for api.internal",
      severity: "low" as const,
    },
    {
      time: "14:18:33",
      msg: "Anomalous outbound traffic from PID 7888",
      severity: "critical" as const,
    },
    {
      time: "14:15:00",
      msg: "Firewall rules updated — 3 new entries",
      severity: "low" as const,
    },
  ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setHealthScore((prev) =>
        Math.max(90, Math.min(100, prev + (Math.random() - 0.5) * 0.3))
      );
    }, 3000);
  ...
  const terminateProcess = (pid: number) => {
    setProcesses((prev) =>
      prev.map((p) =>
        p.pid === pid ? { ...p, status: "blocked" as const } : p
      )
    );
    ...
    setActivityFeed((prev) => [
      {
        ...
        msg: `Process PID ${pid} terminated by operator`,
        severity: "high" as const,
      },
      ...prev.slice(0, 9),
    ]);
  ...
  const runSimulation = (type: string) => {
    setActivityFeed((prev) => [
      {
        time: new Date().toTimeString().slice(0, 8),
        msg: `[SIMULATION] ${type} attack initiated`,
        severity: "critical" as const,
      },
      ...prev.slice(0, 9),
    ]);
      ...
      <div className="space-y-4 animate-fade-in">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {["DDoS Simulation", "Ransomware Drill", "Intrusion Test"].map(
              (sim) => (
                <button
                  key={sim}
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-md text-xs font-mono bg-secondary hover:bg-accent tactical-border transition-colors"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-md bg-card tactical-border flex flex-col items-center justify-center">
            <h3 className="text-xs font-mono text-muted-foreground uppercase tracking-wider mb-2 self-start">
              Network Radar
            </h3>
            <RadarWidget />
          </div>
              ...
              {processes.map((p) => (
                <div
                  ...
                  className={`flex items-center justify-between text-xs font-mono p-2 rounded-sm ${
                    p.status === "suspicious"
                      ? "bg-destructive/10 tactical-border"
                      : p.status === "blocked"
                      ? "bg-muted opacity-50"
                      : "bg-muted"
                  }`}
